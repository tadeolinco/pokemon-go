...
exports.create = (pokemon, cb) => {
    db.query('INSERT INTO pokemon VALUES(0, :entity, :name, :cp, :type1, :type2, :level, :date_caught, :user_id, :gym_id)', pokemon, (err, rows) => {
        if (err) throw err;
...
exports.update = (pokemon, cb) => {
    ...
    db.query('UPDATE pokemon SET entity=:entity, name=:name, cp=:cp, type1=:type1, type2=:type2, level=:level, date_caught=str_to_date(:date_caught, "%Y-%m-%d"), user_id=:user_id, gym_id=:gym_id WHERE pokemon_id=:pokemon_id', pokemon, (err, rows) => {
        if (err) throw err;
        return cb(pokemon);
    });
}
